...
const NavBar = ({activeNo, handleCallback, ...props}) => {
  ...
  return (
    <nav>
      ...
      <AnimatedLink number={4} styleClass={`color-4 ${activeNo == 4 ? "active" :""}`} to="/photography" isActive={activeNo == 4} animationClass="selected" callback={handleCallback}>Photography</AnimatedLink>
      ...
      <AnimatedButton styleClass="color-0" onClick={toggleMute} > {`${isMuted ? "🔇" : "🔈"}`}</AnimatedButton>
    </nav>
  );
}
